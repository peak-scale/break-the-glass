...
```shell
KIND_K8S_VERSION="v1.33.0" make e2e-build
```
...
The Plugin is located in the [`plugin/`](../plugin/) directory. To make use of the plugin locally:
...
2. Make it accessible
   ...
   ```shell
   # (Ensure /usr/local/bin is in your $PATH, Can also be any other PATH)

   chmod +x kubectl-accessdev
   mv kubectl-accessdev /usr/local/bin
   ```
